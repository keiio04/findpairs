...
async function requestNotifPermission(){
  if(!('Notification' in window)){ pwaToast('❌ Notifications not supported.'); return; }
  try{
    ...
    if(permission==='granted'){
      ...
      setTimeout(()=>showLocalNotif('FLIP & MATCH ⛧','Welcome! The creatures are waiting for you…'),1800);
      scheduleReminder();
      ...
      if(btn){
        ...
        btn.title='Enable notifications in your browser/device settings.';
        btn.onclick=()=> pwaToast('🔕 Blocked — enable notifications in your browser/device settings.');
      }
  ...
  }catch(e){ console.error('[PWA] Permission request error:',e); }
}
